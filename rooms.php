...
<body>
  ...
  <!-- Mulai Perulangan -->
  <?php
    $query = "SELECT nama, alamat, tanah, bangunan, tidur, mandi, mobil, att, gambar FROM data_rumah";
    $result = mysqli_query($conn, $query);

    if ($result) {
        $index = 0; // Variabel untuk melacak indeks
        while ($data = mysqli_fetch_assoc($result)) {
            $index++; ?>

  <section class="room introduction_area <?= $index % 2 == 0 ? 'grey-background' : ''; ?>">
    <div class="container">
      <div class="row introduction_inner">
        ...
        <div class="col-md-5">
          <div class="introduction_img">
            <img src="images/home/<?= $data['gambar']; ?>" alt="Foto <?= $data['nama']; ?>"
              style="width: 300px; height: 300px; object-fit: cover; border-radius: 10%;">
          </div>
        </div>
      </div>
    </div>
